...
export default function ImprimirFichaPage() {
  ...
  const { fichasTecnicas, borradoresImpresion, fetchFichasTecnicas } = useOperacionesStore();
  ...
  useEffect(() => {
    const checkAuth = () => {
      // Si el store se ha cargado (isAuthenticated es real)
      setIsHydrated(true);
    };
    checkAuth();
  }, []);
  ...
  useEffect(() => {
    if (isHydrated && !isAuthenticated) {
      // Solo redirigir si estamos seguros de que no está autenticado tras la hidratación
      // router.push("/login"); // Comentado para evitar bucles si la hidratación falla
    }
  ...
  if (!ficha) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 gap-4">
        ...
        <p className="font-black text-slate-400 uppercase text-[10px] tracking-widest">Cargando formato...</p>
      </div>
    );
  }

  const borrador = borradoresImpresion[id] || {};
  ...
  return (
    <div className="min-h-screen bg-slate-200 py-8 print:p-0 print:bg-white">
      ...
      <div className="fixed top-6 right-6 flex gap-3 print:hidden z-50">
        ...
        <Button 
          className="bg-accent hover:bg-accent/90 text-white font-black text-xs uppercase shadow-xl px-8"
