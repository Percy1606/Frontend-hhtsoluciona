...
export function SecureDeleteModal({ isOpen, onClose, entityId, entityName }: SecureDeleteModalProps) {
  const [password, setPassword] = useState("");
  const { secureRemoveInsumo, loading } = useLogisticaStore();
  ...
  const handleConfirm = async () => {
    ...
    try {
      await secureRemoveInsumo(entityId, password);
      toast.success("Insumo Eliminado", { description: `"${entityName}" ha sido removido permanentemente.` });
      setPassword("");
      onClose();
    } catch (error: any) {
      toast.error("Acceso Denegado", { 
        description: error.message || "Contraseña incorrecta o error en la eliminación." 
      });
    }
  };
            ...
            <div className="bg-red-50 p-4 rounded-xl border border-red-100 flex items-start gap-4">
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-xs font-bold text-slate-600 leading-relaxed">
                        Estás a punto de eliminar el insumo <span className="text-red-700 font-black">"{entityName}"</span>. 
                        Esta acción borrará también todo su historial de movimientos y compras.
                    </p>
                </div>
            </div>
